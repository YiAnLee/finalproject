...
def edge_cluster( lines, fileindex, db_eps=30, db_min_samples=5, dump=False, filename="", img=None ):
	...
	for i in range(0,num_of_lines):
		...
		rho_avg, theta_avg = np.average(lines[indices], axis=0)
...
def get_yellow( img, dump=False, filename="" ):
	return get_range( img, np.array([20, 100, 100], np.uint8), np.array([30, 255, 255], np.uint8), cv2.COLOR_BGR2HSV, dump=dump, filename=filename )
...
if __name__ == "__main__":
	...
	line_whites  = get_range( img, np.array([0,100,0], np.uint8), np.array([255,255,255], np.uint8), cv2.COLOR_BGR2HLS, dump=True, filename="result/lined_white.jpg" )
	line_reds    = get_range( img, np.array([150,0,0], np.uint8), np.array([255,255,255], np.uint8), cv2.COLOR_BGR2HLS, dump=True, filename="result/lined_red.jpg" )
